...
def analyze(
    ...
    with span("pii.pipeline"):
        ...
        cands = _evaluated_candidates(view, enabled, policy, use_ner is not False)
        with span("pii.resolve_spans"):
            cands.extend(_propagate_values(view, cands))
            selected = select(view, cands)
...
__all__ = ["MASK", "analyze", "chunks", "explain"]
